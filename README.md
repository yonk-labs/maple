...
| Rust | `.rs` | ✓ struct/enum/trait + `impl` blocks | ✓ (`X::y` counts as method) | ✓ `use`, `use .. as` | `self.foo()` in `impl T` → T; `T::y()` / `m::T::y()` / `Self::y()` → T. A trait never narrows (`Trait::m(&x)` stays ambiguous); a std type (`Vec::new()`) → unresolved; `x.m()` on a local `let x = T::new()` / `T { .. }` / `x: &T` / typed param → T | ✓ `///` |
| C | `.c` `.h` | ✓ (no classes) | all `func` (C has no methods) | `#include` raw only | — | — |
| C++ | `.cpp` `.cc` `.hpp` `.hh` | ✓ class/struct + out-of-line `X::y` defs | ✓ | `#include` raw only | `this->foo()` → enclosing class (+ one hop to its first base); `x.m()` / `x->m()` on a local `T x;` / `T* x` / `const T& x` / `auto x = new T` → T | — |
| C# | `.cs` | ✓ class/interface/struct/record | ✓ | ✓ `using`, `using X = Y` | `this.Foo()` → enclosing class (+ one hop to its first base); `x.M()` on a local `T x` / `var x = new T()` / typed param → T | ✓ `///` / `/** */` |
| Java | `.java` | ✓ class/interface/enum/record | ✓ | ✓ imports (last segment) | `this.foo()` → enclosing class (+ one hop to its superclass); `x.m()` on a local `T x` / `var x = new T()` / typed param → T | ✓ `/** */` |
| JavaScript | `.js` `.jsx` `.mjs` `.cjs` | ✓ classes + `const x = () =>` arrows | ✓ | ✓ `import {a as b}`, defaults | `this.foo()` → enclosing class (+ one hop to `extends`; `function(){}` rebinds `this`); `Promise`/`Math`/`console`… calls → unresolved unless the file declares the name; `foo()` / `ns.foo()` imported from a relative module (`./x`, `./dir` → index, TS-ESM `./x.js` → `x.ts`) → that file's def; from `node:*`/Node core/a few well-known packages → unresolved; `x.m()` on a local `const x = new T()` → T | ✓ `/** */` |
| TypeScript | `.ts` `.tsx` | ✓ (TS + TSX grammars, one language) | ✓ | ✓ `import {a as b}`, defaults | same as JavaScript, plus `x: T` annotations and `(x: T)` params → T | ✓ `/** */` |
